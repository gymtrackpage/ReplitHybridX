...
export function selectHyroxProgram(assessmentData: AssessmentData): ProgramRecommendation {
  console.log("Selecting program for assessment data:", assessmentData);
  
  // Determine experience level
  let experienceLevel = "beginner";
  let programName = "Beginner Program";
  let reasoningExplanation = "";
  
  const hyroxEvents = assessmentData.hyroxEventsCompleted || 0;
  const fitnessYears = assessmentData.generalFitnessYears || 0;
  const trainingDays = assessmentData.weeklyTrainingDays || 0;
  const background = assessmentData.primaryTrainingBackground || "";
  
  // Basic program selection logic
  if (hyroxEvents === 0 && fitnessYears < 2) {
    experienceLevel = "beginner";
    programName = "Beginner Program";
    reasoningExplanation = "You're new to HYROX and fitness training. This program will build your foundation.";
  } else if (hyroxEvents >= 1 && hyroxEvents <= 2) {
    experienceLevel = "intermediate";
    programName = "Intermediate Program";
    reasoningExplanation = "With some HYROX experience, this program will help you improve your performance.";
  } else if (hyroxEvents >= 3 || fitnessYears >= 5) {
    experienceLevel = "advanced";
    programName = "Advanced Program";
    reasoningExplanation = "Your experience level allows for advanced training methods and higher intensity.";
  }
  
  // Adjust based on training background
  if (background === "powerlifting" || background === "strength") {
    programName = "Strength Program";
    reasoningExplanation = "Your strength background suggests focusing on maintaining strength while building endurance.";
  } else if (background === "running" || background === "endurance") {
    programName = "Runner Program";
    reasoningExplanation = "Your running background suggests focusing on strength while maintaining endurance.";
  }
  
  // Check for doubles format
  if (assessmentData.competitionFormat === "doubles" || assessmentData.competitionFormat === "both") {
    programName = "Doubles Program";
    reasoningExplanation = "Doubles competition requires specific training adaptations for partner work.";
  }
  
  const modifications = [];
  
  if (trainingDays < 4) {
    modifications.push("Reduced training frequency to match your availability");
  }
  
  if (assessmentData.equipmentAccess === "minimal") {
    modifications.push("Modified exercises for minimal equipment setup");
  }
  
  if (assessmentData.injuryHistory) {
    modifications.push("Movement modifications for injury prevention");
  }
  ...
  return {
    recommendedProgram: {
      ...
      name: programName,
      description: `A comprehensive ${experienceLevel} HYROX training program`,
    ...
    timeAvailability: `${trainingDays} days per week`,
    specialCategory: assessmentData.competitionFormat || "singles",
    fitnessProfile: {
      runningCapacity: hyroxEvents > 0 ? 7 : 5,
      strengthFoundation: background === "powerlifting" ? 8 : 6,
      movementQuality: fitnessYears > 3 ? 7 : 5,
      workCapacity: trainingDays >= 5 ? 8 : 6,
      stationEfficiency: hyroxEvents >= 3 ? 8 : 5
    }
  };
}
